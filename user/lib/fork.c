...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if (perm & PTE_COW == 0) {
		user_panic("ERROR: Faulting page doesn't have PTE_COW\n");
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	if ((perm & PTE_D) && !(perm & PTE_LIBRARY) && !(perm & PTE_COW)) {
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm & ~PTE_D | PTE_COW);
		syscall_mem_map(0, (void *)addr, 0, (void *)addr, perm & ~PTE_D | PTE_COW);
	} 
	else {
